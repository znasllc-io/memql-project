...
#      workspace repo),
#   3. stamps the product bundle and client repos from templates/bundle
...
cap_spec_param "go-module"      "REMOVED (flag): the carrier (Go module + go.work) variant is archived on branch 'archive/carrier-payload'; passing this now fails with exit 2. Future bespoke-Go (a thin bff plugin) is tracked in issue #11."
...
# Which product payloads to stamp: the DSL-first bundle + client (platform
# consolidation memql#2472) -- no product Go, no go.work, runtime DSL delivery.
# The carrier (Go module + go.work) variant was archived on branch
# 'archive/carrier-payload' (issue #9); bespoke-Go is now tracked in issue #11.
...
# stamp_payloads -- templates/bundle -> <product>-bundle/,
...
# exist (engine + cockpit). The DSL-first bundle and the client repo are not
# Go modules and never appear.
...
function append_product_gitignore() {
    local dir line
    # Ignore whatever was actually stamped (bundle+client) -- not a hardcoded set.
...
function print_dry_run_plan() {
    ...
    cap_info "product model:    DSL-first bundle (no product Go)"
    for name in "${TEMPLATE_DIRS[@]}"; do
...
function main() {
    ...
    # The carrier (Go module + go.work) variant was archived on branch
    # 'archive/carrier-payload' (issue #9). --go-module no longer stamps a
    # carrier; it fails so callers check out that branch for the full carrier,
    # or track the future bespoke-Go (a thin bff plugin) path in issue #11. The
    # default bundle+client stamp below is unchanged.
    if [[ -n "$(cap_flag go-module)" ]]; then
        cap_fail 2 "--go-module is removed: the carrier (bespoke-Go) variant is archived on branch 'archive/carrier-payload' (check it out for the full carrier); the future bespoke-Go bff plugin is tracked in issue #11. The default stamp is the DSL-first bundle + client."
    fi
    TEMPLATE_DIRS=("bundle" "client")
    STAMPED_REPOS=()
