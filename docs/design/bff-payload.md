...
- **Name convention:** `<product>-bff` in the product registry, parallel to the
  existing `<product>-dsl-bundle` and the client surfaces under `clients/`.
  The `images:`
  ...
  - **staging/prod** overlays: `newName: <registry>/<product>-bff`
    ...
    the client surfaces are pinned today.
...
- **Digest gate composition -- ALREADY GENERALIZED (memql#3325).** This section
  originally called for a CI change: the deploy-lane digest gate hardcoded the
  product-image set as `(-dsl-bundle | -client)`, so a product bff would have
  been a **third** image the pattern missed, and adding `-bff` to the literal
  would have broken **no-Go** products (whose bff is the tag-pinned engine
  `memql-bff`). The plural-`clients/` move made the same generalization
  necessary for a second client surface, so the gate now reads: *every image
  that is not an engine-registry image (`acrmemql.azurecr.io/...`, tag-pinned to
  `ENGINE_REF`) must be digest-pinned.* That already covers the optional bff
  with no further CI diff, and keeps the `>= 2` floor (bundle + at least one
  client surface) intact, with the bff an
...
`shellcheck`, `capability-conformance`, `clients`, `deploy`, `dsl` are unchanged.
...
that touches no `clients/` sees the `clients` lane skip.
...
| `scripts/sdk-gen/main.go` | optional `bff/scripts/sdk-gen` | `sdk/gen` (`gen.Options` / `gen.Generate`) still exists in the engine. Only if the product publishes a typed TS SDK; the current template client surface is self-contained (no SDK), so this is out of scope until a product needs a published SDK. |
...
5. **SDK generation demand.** Ship the `sdk-gen` salvage now (behind an
   ...
   SDK? Current lean: defer -- the template client surface is self-contained.
...
- **No published SDK** by default (the client surface stays self-contained).
